...
//    "mailpoll-mailosaur-1m" ON services 
//BEGIN 
//SELECT 
...
//INTO 
//    services."1year"."mailpoll-mailosaur-rollup" 
//FROM 
//    services."1hour"."mailpoll-mailosaur" 
//GROUP BY 
//    time(1m), environment, endpoint, handler, method, status_code 
//END

option task = {
    name: "mailpoll-mailosaur-1m",
    every: 1m,
    delay: 5m,
}

base = from(bucket: "services/1hour") |> range(start: -10m) |> filter( fn: (r) => r._measurement == "mailpoll-mailosaur")
toBucket = (table=<-) => table |> set(key: "_measurement", value: "mailpoll-mailosaur-rollup" ) |>  to(org: "0492cb87e4ea2a22", bucket: "services/1year") 
