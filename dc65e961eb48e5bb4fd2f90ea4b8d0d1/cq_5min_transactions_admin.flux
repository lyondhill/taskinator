...
//     cq_5min_transactions_admin ON insights 
// RESAMPLE EVERY 5m FOR 2d 
// BEGIN 
// SELECT 
//     count(amount) AS transaction_count, sum(usd_amount) AS usd_amount, sum(amount) AS amount 
// INTO 
//     insights."7_days"."5min_transactions_admin" 
// FROM 
//     insights.autogen.transactions 
// GROUP BY 
//     time(5m), gateway_type, currency_code, transaction_type, transaction_state 
// END

option task = {
    name: "cq_5min_transactions_admin",
    every: 5m,
}

base = from(bucket: "insights/autogen") |> range(start: -2d) |> filter(fn: (r) => r._measurement = "transactions") |> window(every: 5m)
finish = (table=<-) => table |> group(by: ["organization_key", "environment_key", "gateway_type", "currency_code", "transaction_type", "transaction_state"]) |> set(key: "_measurement", value: "5min_transactions_admin" ) |>  to(org: "b5e1b39b36674db8", bucket: "insights/autogen") 
